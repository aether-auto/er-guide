...
//
// evaluateCalcCorrectGraph() and decodeRegulationData() are ported verbatim
// (only import paths adapted). They expand the compact regulation JSON
// (public/regulation-vanilla-v1.14.js) into per-upgrade-level Weapon objects.

import { allDamageTypes, allStatusTypes, AttackPowerType } from './attackPowerTypes'
import type { WeaponType } from './weaponTypes'
import type { AttackElementCorrect, Weapon } from './weapon'
...
/**
 * Compact JSON representation of a weapon with things like AttackElementCorrect
 * represented by IDs. Converted to a denormalized Weapon object on the client.
 */
...
/** Decode the game regulation data into a convenient JavaScript object */
  ...
  return weapons.map(
    ({
      ...
      if (reinforceParams == null) {
        throw new Error(`No ReinforceParamWeapon found for id=${reinforceTypeId} weapon=${weapon.name}`)
      }
