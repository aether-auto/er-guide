...
//
// getWeaponAttack() and adjustAttributesForTwoHanding() are ported verbatim
// (only import paths adapted, and getWeaponAttack exported as a named export).
...
/**
 * Adjust a set of character attributes to take into account the 50% Strength
 * bonus when two-handing a weapon.
 */
  ...
  for (const attackPowerType of [...allDamageTypes, ...allStatusTypes]) {
    ...
    if (baseAttackPower || weapon.sorceryTool || weapon.incantationTool) {
      ...
      if (ineffectiveAttributes.some((attribute) => scalingAttributes[attribute])) {
        // If the requirements for this damage type are not met, a penalty is subtracted instead of a
        // scaling bonus being added.
      ...
      // The final scaling multiplier modifies the attack power for this damage type as a percentage
      // boost, e.g. 0.5 adds +50% of the base attack power.
      if (baseAttackPower) {
